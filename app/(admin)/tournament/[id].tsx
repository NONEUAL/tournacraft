...
type Tab = "teams" | "bracket" | "settings";
  ...
  useEffect(() => {
    const ch = supabase.channel(`tournament-${id}`)
      .on("postgres_changes",
        ...
        (payload) => setTournament(prev => prev ? { ...prev, ...payload.new } : prev)
      ).subscribe();
  ...
  return (
    <View style={styles.page}>
      ...
      <View style={styles.topBar}>
        ...
        <Text style={styles.breadcrumbCurrent} numberOfLines={1}>{tournament.name}</Text>
      </View>
      ...
      <View style={styles.tabBar}>
        {TABS.map((tab) => {
          ...
          return (
            ...
            >
              <Feather name={tab.icon} size={13} color={active ? "#A5A0FF" : hovered ? "#64748B" : "#374151"} />
              <Text style={[styles.tabLabel, active && styles.tabLabelActive,
                            !active && hovered && styles.tabLabelHovered]}>
                {tab.label}
              </Text>
...
const styles = StyleSheet.create({
  page:      { flex: 1, backgroundColor: "#080812" },
  centered:  { flex: 1, alignItems: "center", justifyContent: "center", backgroundColor: "#080812", gap: 12 },
  errorText: { color: "#64748B", fontSize: 15 },
  backBtn:   { paddingHorizontal: 16, paddingVertical: 8, backgroundColor: "#0C0C1A", borderRadius: 8, borderWidth: 1, borderColor: "#13132A" },
  backBtnText: { color: "#6C63FF", fontSize: 13, fontWeight: "600" },
  ...
  breadcrumbText:    { color: "#374151", fontSize: 12, fontWeight: "500" },
  breadcrumbCurrent: { color: "#64748B", fontSize: 12, fontWeight: "500", flex: 1 },
  ...
  tournamentName: { color: "#F1F5F9", fontSize: 20, fontWeight: "700", letterSpacing: -0.3 },
  metaRow:        { flexDirection: "row", alignItems: "center", gap: 6, marginTop: 3 },
  gameType:       { fontSize: 10, fontWeight: "700", letterSpacing: 1 },
  metaSep:        { color: "#1F2937", fontSize: 12 },
  metaDate:       { color: "#374151", fontSize: 12 },
  ...
  tabActive:        {},
  tabLabel:         { color: "#374151", fontSize: 13, fontWeight: "500" },
  tabLabelActive:   { color: "#A5A0FF", fontWeight: "600" },
  tabLabelHovered:  { color: "#64748B" },
  tabUnderline:     { position: "absolute", bottom: -1, left: 0, right: 0, height: 2, backgroundColor: "#6C63FF", borderRadius: 1 },
